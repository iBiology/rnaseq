...
params: 
  ...
  indfilter: FALSE
  cookscutoff: FALSE
  countFile: '/Users/fei/Projects/MoleculeIO/tmp/feature.count.txt'
  annoFile: '/Users/fei/Projects/MoleculeIO/tmp/annotation.txt'
  metaFile: "/Users/fei/Projects/MoleculeIO/tmp/meta.xlsx"
  contrastFile:  "/Users/fei/Projects/MoleculeIO/tmp/contrast.xlsx"
  goiFile: "/Users/fei/Projects/MoleculeIO/tmp/goi.txt"
  blackSamples: "EV_Combo_2, EV_Veh_3, p53OE_Veh_2, p53OE_Combo_3"
---
...
dir.create("/Users/fei/Projects/MoleculeIO/tmp/rnk", showWarnings = FALSE)
...
volcanoplot <- function(res, anno, name='name', 
  ...
  res0 <- res
  res<-merge(data.frame(res0), anno, by.x = 0, by.y=0, all.x=T, sort=F)
  ...
  EnhancedVolcano(res,
                       ...
                       y = 'padj',
                       title = name,
                       subtitle = paste("Down:", neg.n, ", Up:", pos.n, sep = ""),
...
Heatmap <- function(df, nclass=2, fname="heatmap", main="title"){
  if (dim(df)[1] > nclass){
    ...
    print(head(df))
    print("Head")
    print(head(df[, c("EV_Veh_1", "EV_Veh_2", "EV_Combo_1", "EV_Combo_3")]))
    # my_sample_col <- my_sample_col[c("EV_Veh_1", "EV_Veh_2", "EV_Combo_1", "EV_Combo_3")]
    p <- pheatmap(df[, c("EV_Veh_1", "EV_Veh_2", "EV_Combo_1", "EV_Combo_3")], 
                  # annotation_col = my_sample_col[rownames(my_sample_col) %in% c("EV_Veh_1", "EV_Veh_2", "EV_Combo_1", "EV_Combo_3"), ],
                  main = main,
                  cluster_cols = F, 
                  border_color = NA,
                  cutree_rows = nclass, 
...
process_deseq_res <- function(res="lfcshrink.res", res2="results.res", name='name', anno='anno.df', norm_exp="tpm.df"){
    ...
    ##  Plots
    ggplot(data.frame(res), aes(x=pvalue))+geom_histogram(color="darkblue", fill="lightblue")
    ggsave(paste0(name, '.pvalue.pdf'))
    
    ggplot(data.frame(res), aes(x=padj))+geom_histogram(color="darkblue", fill="lightblue")
    ...
    Heatmap(zscore.df, nclass = 1,
            fname = paste(name, "heatmap", sep="."),
            main = paste(name, " ", dim(zscore.df)[1], " Significant genes LFC >", min_lfc, "FDR <", max_fdr ))
    
    goi <- readTable(goiFile)
    res <- res[(rownames(res) %in% goi$Name),]
    res2 <- res2[(rownames(res2) %in% goi$Name),]
    volcanoplot(res, anno, lfcthresh=min_lfc, sigthresh=max_fdr,
                textcx=.8,  name= paste(name, " GOI (LFC_shrunken)", sep="."))
    volcanoplot(res2, anno, lfcthresh=min_lfc, sigthresh=max_fdr,
                textcx=.8,name= paste(name, " GOI (LFC_raw)", sep="."))
    
    
    resdata.sig <- resdata[(resdata$Name %in% goi$Name), ]
    dim(resdata.sig)
    n1 <- dim(resdata.sig)[2]
    n2 <- dim(norm_exp)[2]
    zscore.df <- zscore(resdata.sig[, (n1-n2+1):n1])
    ...
    colnames(zscore.df) <- gsub (":DESeq2NormalizedCount", "", colnames(zscore.df))
    Heatmap(zscore.df, nclass = 1,
            fname = paste(name, "GOI.heatmap", sep="."),
            main = paste(name, " ", dim(zscore.df)[1], " GOIs"))
}
```
...
meta.df <- meta.df[!(meta.df$SAMPLE_LABEL %in% blackSamples), ]
...
if (!TEST){
  WriteXLS(x = normalized_counts_out, 
           ExcelFileName = 'DESeq2NormalizedCounts.xlsx', row.names = F, SheetNames = 'NormCounts', na = 'NA')
  goi <- readTable(goiFile)
  WriteXLS(x = normalized_counts_out[normalized_counts_out$Name %in% goi$Name, ], 
           ExcelFileName = 'DESeq2NormalizedCounts.xlsx', row.names = F, SheetNames = 'GOI', na = 'NA')
